...
export class FileHandlerService {
  ...
  validator(file: File, options: IFileOptions) {
    if (file != null) {
        ...
        // } else {
          ...
          if (configs != null && configs.length > 0) {
            ...
            configs.forEach(x => {
              ...
              contentTypes += x.contentType + ',';
            });
            fileTypes = fileTypes.substring(0, fileTypes.lastIndexOf(','));
            contentTypes = contentTypes.substring(0, contentTypes.lastIndexOf(','));
            this.fileOptions.acctptedTypes = fileTypes;
            this.fileOptions.contentTypes = contentTypes;

            const tconfigs = configs.filter(x => x.fileType.split(',').includes(ext));
            const contentTypeConfigs = configs.filter(x => x.contentType.split(',').includes(contentType));

            if (tconfigs != null && tconfigs.length > 0 && contentTypeConfigs != null && contentTypeConfigs.length>0) {
      ...
      if (this.fileOptions.acctptedTypes &&
        (!this.fileOptions.acctptedTypes.split(',').includes(ext) || !this.fileOptions.contentTypes.split(',').includes(contentType) ||
  ...
  private showMessage() {
    let sizeError = '';
    if (this.fileOptions.hasOwnProperty('maxSizeKB') && this.fileOptions.maxSizeKB > 0) {
      sizeError = ` & Please upload file within ${this.fileOptions.maxSizeKB/1024} Mb`;
    }
    const message = `Uploaded file must be either ${this.fileOptions.acctptedTypes}${sizeError}.`;
